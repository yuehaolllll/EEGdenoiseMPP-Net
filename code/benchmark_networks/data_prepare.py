...
if __name__ == "__main__":
    ...
    # 1. 配置参数 (根据论文标准设置)
    # 如果你想处理眼电，就用 'EOG'；如果处理肌电，就改为 'EMG'
    NOISE_TYPE = 'EMG'

    # 论文中 EOG 建议 combin_num=10，EMG 建议 combin_num=20
    COMBIN_NUM = 10 if NOISE_TYPE == 'EOG' else 20
    TRAIN_PER = 0.8  # 80% 训练集

    print(f"开始生成 {NOISE_TYPE} 任务数据集...")

    # 2. 检查并加载原始文件
    eeg_file = 'F:\EEG\EEGdenoiseNet\EEGdenoiseNet_pytorch\data\EEG_all_epochs.npy'
    noise_file = 'F:\EEG\EEGdenoiseNet\EEGdenoiseNet_pytorch\data\EMG_all_epochs.npy' if NOISE_TYPE == 'EMG' else 'EMG_all_epochs.npy'

    if not os.path.exists(eeg_file) or not os.path.exists(noise_file):
        print(f"错误: 找不到 {eeg_file} 或 {noise_file}，请确保它们在当前目录下。")
    else:
        EEG_all = np.load(eeg_file)
        noise_all = np.load(noise_file)

        print(f"原始 EEG 形状: {EEG_all.shape}")
        print(f"原始 Noise 形状: {noise_all.shape}")

        # 3. 调用 prepare_data 函数
        (train_x, train_y,
         val_x, val_y,
         test_x, test_y,
         test_std) = prepare_data(EEG_all, noise_all, COMBIN_NUM, TRAIN_PER, NOISE_TYPE)

        # 4. 将生成的数据集保存到本地，方便后续 PyTorch 直接调用
        # 这样你就不用每次都重新运行耗时的数据合成过程了
        save_path = f'./processed_data_{NOISE_TYPE}/'
        if not os.path.exists(save_path):
            os.makedirs(save_path)

        np.save(f'{save_path}train_x.npy', train_x)
        np.save(f'{save_path}train_y.npy', train_y)
        np.save(f'{save_path}val_x.npy', val_x)
        np.save(f'{save_path}val_y.npy', val_y)
        np.save(f'{save_path}test_x.npy', test_x)
        np.save(f'{save_path}test_y.npy', test_y)
        np.save(f'{save_path}test_std.npy', test_std)

        print(f"--- 数据集生成并保存成功！ ---")
        print(f"保存路径: {save_path}")
        print(f"训练集样本数: {len(train_x)}")
        print(f"测试集样本数: {len(test_x)}")
        print("提示: 下一步你可以直接加载这些 .npy 文件进行 PyTorch 训练。")